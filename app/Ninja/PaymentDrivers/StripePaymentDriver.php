...
class StripePaymentDriver extends BasePaymentDriver
{
    ...
    public function gatewayTypes()
    {
        ...
        if ($this->companyGateway && $this->companyGateway->getAchEnabled()) {
    ...
    {
        return $this->companyGateway->getPublishableStripeKey();
    }
    ...
    protected function creatingPaymentMethod($paymentMethod)
    {
        ...
        if (! empty($data['object']) && ($data['object'] == 'card' || $data['object'] == 'bank_company')) {
            $source = $data;
    ...
    public function removePaymentMethod($paymentMethod)
    {
        ...
        if (! $paymentMethod->relationLoaded('company_gateway_token')) {
            $paymentMethod->load('company_gateway_token');
        }

        $response = $this->gateway()->deleteCard([
            'customerReference' => $paymentMethod->company_gateway_token->token,
    ...
    {
        $clientId = $this->companyGateway->getPlaidClientId();
        $secret = $this->companyGateway->getPlaidSecret();
        ...
        try {
            $subdomain = $this->companyGateway->getPlaidEnvironment() == 'production' ? 'api' : 'tartan';
    ...
    {
        $apiKey = $this->companyGateway->getConfig()->apiKey;
    ...
    public function handleWebHook($input)
    {
        ...
        if ($eventType == 'charge.failed' || $eventType == 'charge.succeeded' || $eventType == 'charge.refunded') {
        ...
        } elseif ($eventType == 'customer.source.updated' || $eventType == 'customer.source.deleted' || $eventType == 'customer.bank_company.deleted') {
            ...
            if ($eventType == 'customer.source.deleted' || $eventType == 'customer.bank_company.deleted') {
