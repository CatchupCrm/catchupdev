...
class Company extends Eloquent
{
    ...
    {
        return $this->hasMany('App\Models\CompanyToken');
    }
    ...
    {
        return $this->hasMany('App\Models\CompanyGateway');
    }

    /**
     * @return \Illuminate\Database\Eloquent\Relations\HasMany
     */
    public function bank_companies()
    {
    ...
    {
        if (! $this->relationLoaded('company_gateways')) {
            $this->load('company_gateways');
        }

        if ($gatewayId) {
            return $this->getGatewayConfig($gatewayId) != false;
        } else {
            return count($this->company_gateways) > 0;
        }
    }
    ...
    {
        $companyGateway = $this->company_gateways[0];
    ...
    */
    ...
    public function getGatewayByType($type = false)
    {
        ...
        foreach ($this->company_gateways as $companyGateway) {
            if (! $type) {
    ...
    public function availableGatewaysIds()
    {
        ...
        foreach (Cache::get('gateways') as $gateway) {
            $paymentDriverClass = CompanyGateway::paymentDriverClass($gateway->provider);
    ...
    {
        return $this->company_gateways()->pluck('gateway_id')->toArray();
    }
    ...
    {
        foreach ($this->company_gateways as $gateway) {
    ...
    protected function calculateLogoDetails()
    {
        ...
        } elseif ($disk->exists($this->company_key.'.jpg')) {
    ...
    {
        foreach ($this->company_tokens as $token) {
    ...
    public function getPlanDetails($include_inactive = false, $include_trial = true)
    {
        ...
        if ($use_plan) {
            return [
                'company_id' => $this->corporation->id,
    ...
    public function hasMultipleCompanys()
    {
        return $this->corporation->companies->count() > 1;
    }
