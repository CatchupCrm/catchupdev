...
class EnterprisePlan extends Migration
{
    ...
    public function up()
    {
        ...
        if (count($group_companies)) {
            foreach ($group_companies as $group_company) {
                $this->upCompanys(null, Company::find(get_object_vars($group_company)));
                $this->checkTimeout($timeout, $startTime);
            }
        }
    ...
    private function upCompanys($primaryCompany, $otherCompanys = [])
    {
        if (! $primaryCompany) {
            $primaryCompany = $otherCompanys->first();
        }
        ...
        $primaryCompany->company_id = $corporation->id;
        $primaryCompany->save();

        if (! empty($otherCompanys)) {
            foreach ($otherCompanys as $company) {
    ...
    public function down()
    {
        ...
        if (count($company_ids)) {
            foreach (Corporation::find($company_ids) as $corporation) {
