...
@section('content')
    ...
    <div class="panel-body">
            ...
            <div class="form-padding-right">
                ...
                @else
                    {!! Former::select('bank_id')
                            ...
                            ->blockHelp(trans('texts.bank_companies_help', ['link' => OFX_HOME_URL]))  !!}
                @endif
        ...
        <div class="col-lg-12 col-sm-12" data-bind="visible: page() == 'setup'" style="display:none">
            <table class="table companies-table" style="width:100%">
                ...
                <tbody data-bind="foreach: bank_companies">
                    <tr>
                        <td>
                            <input type="text" class="form-control" data-bind="value: company_name, valueUpdate: 'afterkeydown', attr: {name: 'bank_companies[' + $index() + '][company_name]'}"/>
                            <input type="text" style="display:none" data-bind="value: hashed_company_number, attr: {name: 'bank_companies[' + $index() + '][hashed_company_number]'}"/>
                        </td>
                        <td data-bind="text: masked_company_number"></td>
                        <td data-bind="text: balance"></td>
                        <td style="text-align:center">
                            <input type="checkbox" value="1"
                                data-bind="checked: includeCompany, attr: {name: 'bank_companies[' + $index() + '][include]'}"/>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="col-lg-12 col-sm-12" data-bind="visible: page() == 'import'" style="display:none">
            ...
            <div data-bind="foreach: bank_companies">
                ...
                <table class="table companies-table" style="width:100%;">
                    ...
                    <tbody data-bind="foreach: filteredTransactions">
                        <tr>
                            <td style="text-align:center">
                                <input type="checkbox" value="1"
                                    data-bind="checked: includeTransaction, attr: {name: 'bank_companies[' + $index() + '][include]'}"/>
                            </td>
                            <td>
        ...
        {!! Former::actions(
            count(Cache::get('banks')) > 0 ?
                Button::normal(trans('texts.cancel'))
                    ...
                    ->large()
                    ->asLinkTo(URL::to('/settings/bank_companies'))
    ...
    function validate() {
        ...
        $.post('{{ URL::to('/bank_companies/validate') }}', $('.main-form').serialize())
            .done(function(data) {
    ...
    function save() {
        model.errorStr(false);
        model.isLoading(true);
        $.post('{{ URL::to('/bank_companies') }}', $('.main-form').serialize())
            .done(function(data) {
                data = JSON.parse(data);
                if (!data || !data.length) {
                    model.isLoading(false);
    ...
    function loadTransactions(data) {
        model.bank_companies.removeAll();
        for (var i=0; i<data.length; i++) {
            ...
            model.bank_companies.push(company);
        }
    }
    ...
    function saveExpenses() {
        ...
        $.ajax({
            url: '{{ URL::to('/bank_companies/import_expenses') }}' + '/' + model.bank_id(),
            type: 'POST',
    ...
    var TransactionModel = function(data) {
        ...
        self.vendor.pretty = ko.computed({
            ...
            write: function (value) {
                this.vendor(value);
                for (var i=0; i<model.bank_companies().length; i++) {
                    var company = model.bank_companies()[i];
                    var transactions = company.transactions();
    ...
    var ViewModel = function() {
        ...
        self.bank_companies = ko.observableArray();
        ...
        self.includedTransactions = ko.computed(function() {
            var data = [];
            for (var i=0; i<self.bank_companies().length; i++) {
                var company = self.bank_companies()[i];
