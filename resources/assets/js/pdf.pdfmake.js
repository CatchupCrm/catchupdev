...
NINJA.invoiceDocuments = function(invoice) {
    if (invoice.company.invoice_embed_documents != '1') {
        return [];
    }
...
NINJA.accountDetails = function(invoice) {
    ...
    if (invoice.features.invoice_settings && company.invoice_fields) {
        var fields = JSON.parse(company.invoice_fields).account_fields1;
    } else {
        var fields = [
...
NINJA.accountAddress = function(invoice) {
    var company = invoice.company;
    if (invoice.features.invoice_settings && company.invoice_fields) {
        var fields = JSON.parse(company.invoice_fields).account_fields2;
    } else {
        var fields = [
            'company.address1',
            'company.address2',
...
NINJA.renderClientOrAccountField = function(invoice, field) {
    ...
    if (field == 'company.company_name') {
        return {text:company.name, style: ['accountName']};
